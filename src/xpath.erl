...
compile_xpath(Expr) ->
    xpath_parser:compile_xpath(Expr).
...
execute(XPathString,Doc,Functions) when is_list(XPathString) ->
    XPath = xpath_parser:compile_xpath(XPathString),
...
convert_xpath_function_args(Args, [{'*', Spec}], Acc) ->
    NewArgs = [xpath_utils:convert(Arg,Spec) || Arg <- Args],
    lists:reverse(Acc) ++ NewArgs;
convert_xpath_function_args([Arg | Args], [Spec | Specs], Acc) ->
    NewAcc = [xpath_utils:convert(Arg,Spec) | Acc],
...
handle_predicate_result(PredicateResult, _) ->
    xpath_utils:boolean_value(PredicateResult).
...
comp_fun('=') -> 
    fun 
        (A,B) when is_number(A) -> A == xpath_utils:number_value(B);
        (A,B) when is_number(B) -> xpath_utils:number_value(A) == B;
        (A,B) when is_boolean(A) -> A == xpath_utils:boolean_value(B);
        (A,B) when is_boolean(B) -> xpath_utils:boolean_value(A) == B;
        (A,B) -> xpath_utils:string_value(A) == xpath_utils:string_value(B)
    end;
...
comp_fun('>') ->
  fun(A,B) -> 
    xpath_utils:number_value(A) > xpath_utils:number_value(B)
  end;
comp_fun('<') ->
  fun(A,B) -> 
    xpath_utils:number_value(A) < xpath_utils:number_value(B)
...
bool_fun('and') ->
    fun(A, B) ->
            xpath_utils:boolean_value(A)
                andalso xpath_utils:boolean_value(B)
    end;
bool_fun('or') ->
    fun(A, B) ->
            xpath_utils:boolean_value(A)
                orelse xpath_utils:boolean_value(B)
    end.
...
arith('+', Arg1, Arg2) ->
    xpath_utils:number_value(Arg1)
		+ xpath_utils:number_value(Arg2);
arith('-', Arg1, Arg2) ->
	xpath_utils:number_value(Arg1)
		- xpath_utils:number_value(Arg2);
arith('*', Arg1, Arg2) ->
...
arith('mod', Arg1, Arg2) ->
	xpath_utils:number_value(Arg1)
		rem xpath_utils:number_value(Arg2).
