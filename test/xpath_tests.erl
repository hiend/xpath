...
unicode_test() ->
    ...
    Doc = xpath_html:parse(DocBin),
    ?assertEqual([<<"unicode-class">>],
                 xpath:execute("/html/body/div[@class='юникод']/text()", Doc)),
    xpath:compile_xpath("/html/body/processing-instruction('юникод')").
...
do_test({File,Cases},UserFunctions) ->
    ...
    lists:map(fun({Expr,Expected}) ->
                      R = xpath:execute(Expr,Doc,UserFunctions),
                      { File ++ " " ++ Expr, ?_assertEqual(Expected, R) };
                 ({Expr, MapResFun, Expected}) ->
                      R = [MapResFun(E) || E <- xpath:execute(Expr,Doc,UserFunctions)],
                      { File ++ " " ++ Expr, ?_assertEqual(Expected, R) }
              end, Cases).
