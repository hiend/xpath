...
-spec string_value(xpath:indexed_xpath_return()) -> binary().
...
-spec node_set_value(xpath:indexed_xpath_return()) -> [xpath:indexed_html_node()].
node_set_value(List) when is_list(List) ->
    List;
node_set_value(N) ->
    throw({node_set_expected,N}).

-spec number_value(xpath:indexed_xpath_return() | binary()) -> number().
...
-spec boolean_value(xpath:indexed_xpath_return()) -> boolean().
boolean_value([]) ->
    false;
boolean_value([_|_]) ->
    true;
